...
var _claw : ClimberClaw
const rope_local_offset := Vector3.RIGHT * 0.5
const rope_world_offset := Vector3.DOWN * 0.5
...
func handle_primary_xr_float(button, value):
    ...
    if button == "trigger":
        ...
        if newstate != primary_trigger:     
            # rising edge
            if newstate:     
                # Most recent trigger press controller always becomes the active one. If the other controller is already
                # active, send a trigger release event first to dismiss the grappler.

                if active_controller == secondary_controller:
                    var release_event = InputEventJoypadMotion.new()
...
func handle_secondary_xr_float(button, value):
    ...
    if button == "trigger":
        ...
        # only send events on changes above threshold
        if newstate != secondary_trigger:     
            # rising edge
            if newstate:     
                # Most recent trigger press controller always becomes the active one.
                # If the other controller is already active, send a trigger release event first to dismiss the grappler.
