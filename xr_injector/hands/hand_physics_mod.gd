...
## LXE97: implemented some simple physics
...
@export var max_controller_separation := 2.0
...
func _ready() -> void:
	...
	resize(XRServer.world_scale)
	force_update_transform()

func resize(factor: float):
	$HandMesh.scale = Vector3.ONE * factor
	$HandMesh.position *= factor
...
func get_camera_to_controller_blocked_transform(target_transform: Transform3D) -> Transform3D:
	var result := target_transform
	result.basis = result.basis.orthonormalized()

	var from := xr_camera.global_position
	var to := target_transform.origin
	var dir := to - from
	...
	# Temporarily test from the camera position
	var test_transform := global_transform
	test_transform.origin = from
	test_transform.basis = result.basis

	var old_transform := global_transform
	global_transform = test_transform
	force_update_transform()

	var collision := move_and_collide(dir, true)

	global_transform = old_transform
	force_update_transform()

	if collision:
		result.origin = collision.get_position() - dir.normalized() * hand_surface_margin
	else:
		result.origin = to
